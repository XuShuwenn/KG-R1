...
def _extract_question_and_entities(sample: Dict[str, Any]) -> Tuple[str, List[str]]:
    """
    Extract question string and initial entities from a VERL-style sample dict.

    Supports the following structures:
    - sample["question"]
    - sample["raw_question"]
    - sample["extra_info"]["question"]
    - sample["prompt"][0]["content"] containing "Question: ..."
    """
    question_candidates = [
        sample.get("question"),
        sample.get("raw_question"),
    ]

    extra_info = sample.get("extra_info") if isinstance(sample.get("extra_info"), dict) else {}
    if extra_info:
        question_candidates.append(extra_info.get("question"))

    question_text = None
    for cand in question_candidates:
        if isinstance(cand, str) and cand.strip():
            question_text = cand.strip()
            break

    if question_text is None:
        prompt_field = sample.get("prompt")
        if isinstance(prompt_field, list) and prompt_field:
            first_msg = prompt_field[0]
            ...
            if isinstance(content, str):
                ...
                if idx != -1:
                    question_text = content[idx + len(marker):].strip()
                else:
    ...
    if isinstance(extra_info, dict):
        entities = extra_info.get("initial_entities")
        if isinstance(entities, list):
            initial_entities = [str(e) for e in entities if e]

    if not initial_entities and "topic_entities" in sample:
        topic_entities = sample["topic_entities"]
        if isinstance(topic_entities, list):
            initial_entities = [str(e) for e in topic_entities if e]

    return question_text, initial_entities


def build_search_prompt(
    question_or_sample: str | Dict[str, Any],
    max_calls: int = 10,
    topic_entities: list | None = None,
) -> str:
    """
    Build the initial search prompt used in KGQA training/eval.

    Args:
        question_or_sample: Either a raw question string (kgqa_agent style)
            or a VERL sample dict (from parquet). When a dict is provided,
            the function will extract the question and initial entities automatically.
        max_calls: maximum number of allowed KG queries.
        topic_entities: optional list of initial entities; if not provided and the
            first argument is a dict, we will use sample["extra_info"]["initial_entities"].
    """
    if isinstance(question_or_sample, dict):
        question_text, initial_entities = _extract_question_and_entities(question_or_sample)
        if topic_entities is None:
            topic_entities = initial_entities
    else:
        question_text = str(question_or_sample)
