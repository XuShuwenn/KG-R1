...
_EXAMPLE_DIR = Path(__file__).parent / "traj_gen_examples"


def _load_random_example() -> str:
    """Load a random example from example1.txt to example10.txt."""
    example_num = random.randint(1, 10)
    example_path = _EXAMPLE_DIR / f"example{example_num}.txt"
...
You are a helpful assistant that generates Supervised Fine-Tuning samples for a Knowledge Graph question-answering task.
Your task is to generate a concise reasoning thought that explains WHY a specific action was chosen at a given step, based on the information you have gathered. Keep it brief and direct, following the style of the example provided.
...
2. The topic entities (starting points)
...
- Briefly explain how the observations from the last step provide the answer
...
def TRAJECTORY_GENERATION_PROMPT() -> str:
