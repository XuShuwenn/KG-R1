...
def process_cwq_data():
    # Load the CWQ datasets
    train_data_path = "data_kg/CWQ/train_simple.json"
    dev_data_path = "data_kg/CWQ/dev_simple.json"
    test_data_path = "data_kg/CWQ/test_simple.json"
    entities_path = "data_kg/CWQ/entities.txt"
    
    # Load entity ID and text mapping
    with open(entities_path, 'r') as f:
        entity_ids = [line.strip() for line in f.readlines()]
    
    entities_text_path = "data_kg/CWQ/entities_text.txt"
    ...
    output_dir = "data_kg/cwq_search_augmented_initial_entities"
    ...
    # Process and save dev data separately
    if os.path.exists(dev_data_path):
        dev_data = load_and_process(dev_data_path, "dev")
        dev_df = pd.DataFrame(dev_data)
        dev_output = os.path.join(output_dir, "dev.parquet")
        dev_df.to_parquet(dev_output, index=False)
        print(f"Saved {len(dev_data)} dev samples to {dev_output}")
    ...
    if 'dev_data' in locals():
        print(f"Sample dev initial entities: {dev_data[0]['extra_info']['initial_entities'] if dev_data else 'None'}")
